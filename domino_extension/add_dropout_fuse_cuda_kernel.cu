...
__global__ void fused_dropout_kernel_vec(
  ...
  printf("fused_dropout_kernel_vec\n");
  printf("a: ");
  for (int i = 0; i < 32; i++) {
    printf("%f ", a.data[i]);
  }
  ...
  for (IndexType linearIndex = idx * VEC; linearIndex < totalElements;
    ...
    printf("after curand_uniform4\n");
    printf("rand: %f, %f, %f, %f\n", rand.x, rand.y, rand.z, rand.w);
    ...
    *residual_value = *reinterpret_cast<LoadT*>(&c.data[linearIndex]);
    ...
    printf("input_src: %f, %f, %f, %f\n", input_src[0], input_src[1], input_src[2], input_src[3]);
    printf("residual_src: %f, %f, %f, %f\n", residual_src[0], residual_src[1], residual_src[2], residual_src[3]);
...
#pragma unroll
    for (int ii = 0; ii < VEC; ii++) {
      r[ii] = input_src[ii] * (&rand.x)[ii] * scale + residual_src[ii];
  ...
  for (IndexType linearIndex = idx; linearIndex < rounded_size;
    ...
    for (int ii = 0; ii < UNROLL; ii++) {
      ...
      if (li < totalElements) {
        ...
        d.data[bOffset] =
            input_src[ii] * (&rand.x)[ii] * scale + residual_src[ii];
...
inline void launcher(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      "fused_dropout",
      [&] {
        printf("AT_DISPATCH_FLOATING_TYPES_AND2\n");
        using accscalar_t = acc_type<scalar_t, true>;
        accscalar_t pa = (accscalar_t)(p);
        // auto input1_info =
        //     cuda::detail::getTensorInfo<scalar_t, index_type>(input1);
        printf("input1.dim(): %d\n", input1.dim());
        printf("input1.size(0): %d\n", input1.size(0));
        printf("input1.size(1): %d\n", input1.size(1));
        printf("input1.size(2): %d\n", input1.size(2));
        printf("input1.stride(0): %d\n", input1.stride(0));
        printf("input1.stride(1): %d\n", input1.stride(1));
        printf("input1.stride(2): %d\n", input1.stride(2));
        printf("input1.data_ptr(): %p\n", input1.data_ptr<scalar_t>());
        index_type sz[MAX_TENSORINFO_DIMS];
        index_type st[MAX_TENSORINFO_DIMS];
        int dims = input1.dim();
        for (int i = 0; i < dims; ++i) {
          sz[i] = input1.size(i);
          st[i] = input1.stride(i);
        }
        cuda::detail::TensorInfo<scalar_t, index_type> input1_info = cuda::detail::TensorInfo<scalar_t, index_type>(
            input1.data_ptr<scalar_t>(), dims, sz, st);
        printf("input1_info: ");
        for (int i = 0; i < 4; i++) {
              printf("%f ", input1_info.data[i]);
        }
        printf("\n");
        auto residual1_info =
        ...
        printf("\n");
        printf("residual1_info: ");
        for (int i = 0; i < 32; i++) {
          printf("%f ", residual1_info.data[i]);
        }
        printf("\n");
        ...
        if (vec_size > 1) {
          ...
          switch (input1_info.dims) {
            case 1:
              ...
              break;
            default:
              if (!input1.is_contiguous() && !residual1.is_contiguous() &&
                  ret1.is_contiguous() && mask1.is_contiguous()) {
              ...
              if (!input2.is_contiguous() && !residual2.is_contiguous() &&
                  ret2.is_contiguous() && mask2.is_contiguous()) {
                fused_dropout_kernel<scalar_t, accscalar_t, index_type, -1, 1>
                    <<<grid, dim_block, 0, stream1>>>(
                        input2_info,
                        residual2_info,
...
std::tuple<Tensor, Tensor, Tensor, Tensor> native_add_dropout_fuse_cuda(
    const Tensor& input1,
    const Tensor& residual1,
    const Tensor& input2,
    const Tensor& residual2,
  ...
  if (train.has_value() && !train.value()) {
    auto ret1 = input1.add(residual1);
    auto ret2 = input2.add(residual2);
  ...
  double p1m = 1. - p;
  printf("native_add_dropout_fuse_cuda\n");
  printf("input1: ");
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 2; j++) {
      for (int k = 0; k < 4; k++) {
  ...
  printf("\n");
  return dropout_cuda<bool>(gen, input1, residual1, input2, residual2, p1m);
}
...
std::tuple<Tensor, Tensor, Tensor, Tensor> native_add_dropout_fuse_2_cuda(
