...
#pragma unroll
    for (int ii = 0; ii < VEC; ii++) {
      r[ii] = input_src[ii] * (&rand.x)[ii] * scale +
          residual_src[ii];
...
std::tuple<Tensor, Tensor, Tensor, Tensor> native_add_dropout_fuse_cuda(
  ...
  return dropout_cuda<bool>(
      gen, input1, residual1, input2, residual2, p1m);
}
...
native_add_dropout_fuse_2_cuda(
